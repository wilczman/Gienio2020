...
def get_position(string):
    ...
    if string.isnumeric():  # TODO
        ...
        name = fileWithChoosing.search(string)
        sheet = wb[name]
    ...
    if number.isnumeric():
        try:
            ...
            for row in range(2, sheet.max_row):
                output += f"{sheet['A' + str(row)].value}: {round(sheet['B' + str(row)].value / 100 * int(number), 3)} {sheet['C' + str(row)].value}\n"
                format_response(output)
            amount_button.place_forget()
            entry_amount.place_forget()
            entry_name.delete(0, 'end')
            entry_amount.delete(0, 'end')
            print_button.place(relx=0.3, rely=0, relwidth=0.4, relheight=1)
        except:
...
if __name__ == "__main__":
    ...
    print_frame = tk.Frame(root, bg='#80c1ff', bd=5)
